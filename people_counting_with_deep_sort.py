...
flags.DEFINE_string('weights', 'bismillah.pt', 'Path to YOLOv8 Weights')
flags.DEFINE_string('video', '1', 'Path to input video or webcam index (0)')
...
def show_counter(frame, title, class_names, obj_count, x_init):
    ...
    for obj_id, count in obj_count.items():
        ...
        cv2.putText(frame, obj_count, (x_init + 135, y_init), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2)
...
def show_total(frame, title,entry,exit):
    ...
    thickness = 2
    position = (10, 150)  # Position to display the text
